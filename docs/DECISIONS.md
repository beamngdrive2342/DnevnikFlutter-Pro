...
- **Decision:** Программно или через AI переводить текст из фотографий решений (ГДЗ) в чистый Markdown/LaTeX для снятия проблем с копирайтом. Вопрос с чертежами пока остается открытым (варианты: кроп оригинальных картинок, GeoGebra, WebView), ручная отрисовка через CustomPaint признана немасштабируемой.
- **Why:** Перевод в текст полностью снимает вопросы по авторскому праву и позволяет рендерить решения нативно и в фирменном стиле. Кастомная отрисовка чертежей на Canvas требует ручного кода под каждую из 3400+ задач, что невозможно.
- **Consequences:** Потребуется найти стабильный рендерер LaTeX (возможно, WebView с KaTeX). Нужно продумать пайплайн парсинга картинок.
