...
## 2026-06-24 - Хранение готовых ответов (ГДЗ) в локальных ассетах
- **Status:** accepted
- **Decision:** Интегрировать 870 фотографий с решениями по Геометрии непосредственно в приложение через `assets/Geometry/`.
- **Why:** Обеспечивает мгновенную загрузку решений без интернета, не требует настройки Firebase Storage и оплаты трафика. Изображения предварительно сжаты.
- **Consequences:** Увеличивает размер исходного `.apk`. Если предметов станет много, придется отказаться от этого решения.
- **Rollback:** Удалить папку `assets/Geometry/` и загружать картинки по URL из Storage.
